...
module.exports = (App) => {
    ...
    try {
      ...
      if (id) {
        ...
        if (product) {
          ...
          res.status(404).send({ Error: 'Could not find any product.' });
        }
      } else {
  ...
  // Update a specific item by the id
    ...
    try {
      ...
      if (validToken) {
        ...
        if (product) {
          ...
          res.status(400).send({ Error: 'Could not find any product.' });
        }
      } else {
        res.status(401).send({ Error: 'Not authorized. The token in the headers is missing or it is not valid.' });
      }
  ...
  // Delete a specific item by the id
